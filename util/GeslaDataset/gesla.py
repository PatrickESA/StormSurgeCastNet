...
class GeslaDataset:
    ...
    def file_to_pandas(self, filename, return_meta=True):
        ...
        with open(self.data_path + filename, "r") as f:
            ...
            # check for repeated data entries (ie duplicate indices)
    ...
    def files_to_xarray(self, filenames):
        ...
        #"""
        """
        netcdf_path = os.path.join(os.path.expanduser('~'), 'Data', 'GESLA', 'netcdf')
        paths   = glob.glob(os.path.join(netcdf_path, '*.nc'))
        data    = xr.open_mfdataset(paths, combine='nested', concat_dim="station", chunks={'date_time':1, 'station':1}, compat='equals', engine='h5netcdf', decode_cf=False)
        """
